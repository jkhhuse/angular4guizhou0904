...
export class RandomUserService {
    ...
    getTotals(): Observable<any[]> {
        return this.http.get(environment.apiApp + '/apiApp' + '/groups/1/application-overview').map(res => res.json());
    }
    ...
    getServiceInstances(pageIndex = 1, pageSize = 5, sortField, sortOrder): Observable<any> {
        ...
        params.set('sortField', sortField);
        params.set('sortOrder', sortOrder);
        return this.http.get(environment.apiService + '/apiService' + '/groups/1/service-instances', {search: params}).map(res => res.json());
    }
    ...
    getAppInstances(pageIndex = 1, pageSize = 5, sortField, sortOrder, tabName): Observable<any> {
        ...
        params.set('sortField', sortField);
        params.set('sortOrder', sortOrder);
        return this.http.get(environment.apiApp + '/apiApp' + '/groups/1/clusters/' + tabName + '/overviews', {search: params}).map(res => res.json());
    }
}
